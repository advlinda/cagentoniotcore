...
static cJSON * cJSON_CreateNetMonInfo(net_info_t * pNetInfo)
{
	...
	cJSON_AddNumberToObject(pNetInfoItem, NET_INFO_INDEX, pNetInfo->index);
...
bool Parser_ParseThrItemInfo(cJSON * jsonObj, net_info_list curNetInfoList, nm_thr_item_t * pThrItemInfo, bool *nIsValidPnt)
{
	bool bRet = false;
			...
			if(pSubItem)
			{
				...
				//检验路径n是否有效
				{
					...
					if((i==4) && (!strcmp(sliceStr[0], DEF_HANDLER_NAME)) && (!strcmp(sliceStr[1], NETMON_INFO_LIST)))
					{
						...
						bool flag = false;
						curNodeNetInfo = curNetInfoList->next;

						while(curNodeNetInfo)
						{
							DWORD index;
							DWORD netSpeedMbps;
							double netUsage;
							...
							{
								if(!strcmp(sliceStr[3], NET_INFO_INDEX))	
								{	
									flag = true;
									break;
								}
								if(!strcmp(sliceStr[3], NET_INFO_SPEED_MBPS))	
								{	
									flag = true;
									break;
								}
								if(!strcmp(sliceStr[3], NET_INFO_USAGE))	
								{	
									flag = true;
									break;
								}
								if(!strcmp(sliceStr[3], NET_INFO_SEND_BYTE))	
								{	
									flag = true;
									break;
								}
								if(!strcmp(sliceStr[3], NET_INFO_RECV_BYTE))	
								{	
									flag = true;
									break;
								}
								if(!strcmp(sliceStr[3], NET_INFO_SEND_THROUGHPUT))	
								{	
									flag = true;
									break;
								}
								if(!strcmp(sliceStr[3], NET_INFO_RECV_THROUGHPUT))	
								{	
									flag = true;
									break;
								}
				...
				if(thrItem)
				{
					...
					for(i=0; i<nCount; i++)
					{
						...
						if(subItem)
						{
							...
							if(Parser_ParseThrItemInfo(subItem, curNetInfoList, &pThrItemNode->thrItem, &nIsValid))//in in out
							{
								if(nIsValid)
								{
...
bool Parser_GetSensorJsonStr(char *pNetInfoList, sensor_info_t * pSensorInfo)
{
	...
	if(pSensorInfo->pathStr != NULL && strlen(pSensorInfo->pathStr))
	{
		...
		if(i>0 && !strcmp(sliceStr[0], DEF_HANDLER_NAME) && !strcmp(sliceStr[1], NETMON_INFO_LIST))
		{
			...
			while(curNode && !flag)
			{
				if(i>2 && !strcmp(sliceStr[2], curNode->netInfo.adapterName))
				{
					if(i>3)
					{
