...
MSG_CLASSIFY_T* GPS_CreateGPS()
{
	MSG_CLASSIFY_T *pRoot = MSG_CreateRoot();
	MSG_CLASSIFY_T *pGPS, *pDevs;
	if(pRoot)
	{
		pGPS =MSG_AddIoTClassify(pRoot, TAG_GPS_ROOT, "1.0", false);
		pDevs = MSG_AddIoTClassify(pGPS, TAG_GPS_DEVICE_GROUP, "1.0", true);
	}
	return pRoot;
}
...
MSG_CLASSIFY_T* GPS_AddDevice(MSG_CLASSIFY_T* gps, char const* device, char const* ver)
{
	...
	target = MSG_FindClassify(gps, TAG_GPS_ROOT);
	if(!target)
		return gpsDev;
...
MSG_ATTRIBUTE_T* GPS_SetModeAttribute(MSG_CLASSIFY_T* device, float value, char* unit)
{
	MSG_CLASSIFY_T* target = NULL;
	MSG_ATTRIBUTE_T* attr = NULL;
	if(!device)
	...
	MSG_SetFloatValue(attr, value, "r", unit);
