...
MSG_CLASSIFY_T* MSG_AddClassify(MSG_CLASSIFY_T *pNode, char const* name, char const* version, bool bArray, bool isIoT)
{
	...
	if(pCurNode)
	{
		...
		if(name)
		{
			strncpy_s(pCurNode->classname, sizeof(pCurNode->classname), name, strlen(name));
		}

		if(version)
		{
			strncpy_s(pCurNode->version, sizeof(pCurNode->version), version, strlen(version));
		}
...
MSG_ATTRIBUTE_T* MSG_AddAttribute(MSG_CLASSIFY_T* pClass, char const* attrname, bool isSensorData)
{
	...
	if(curAttr)
	{
		...
		if(attrname)
		{
			strncpy_s(curAttr->name, sizeof(curAttr->name), attrname, strlen(attrname));
		}
...
bool MSG_SetFloatValue(MSG_ATTRIBUTE_T* attr, float value, char* readwritemode, char *unit)
{
	...
	attr->bRange = false;
	if(readwritemode)
		strncpy_s(attr->readwritemode, sizeof(attr->readwritemode), readwritemode, strlen(readwritemode));
	if(unit)
		strncpy_s(attr->unit, sizeof(attr->unit), unit, strlen(unit));
	return true;
}
...
bool MSG_SetStringValue(MSG_ATTRIBUTE_T* attr, char *svalue, char* readwritemode)
{
	...
	strncpy_s(attr->sv, length+1, svalue, strlen(svalue));
