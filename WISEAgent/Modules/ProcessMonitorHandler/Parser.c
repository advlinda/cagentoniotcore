...
static cJSON * cJSON_CreatePrcMonInfo(prc_mon_info_t * pPrcMonInfo)
{
	...
	cJSON_AddItemToObject(pPMInfoItemArraySub,SWM_BV_FLAG,cJSON_CreateNumber(pPrcMonInfo->isActive));
					...
					while(curNode)
					{
						...
						if(j>0 && curNode->prcMonInfo.prcID == procID && !strcmp(procName[1], curNode->prcMonInfo.prcName))
						{
							...
							//IsActive
								...
								cJSON_AddItemToObject(findVItem,SWM_BV_FLAG,cJSON_CreateNumber(curNode->prcMonInfo.isActive));
							//PID
