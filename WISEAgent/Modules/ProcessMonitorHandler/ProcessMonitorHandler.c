...
*/
	...
	if(checkRet)
	{
		...
		case prc_act_stop:
			{
				sprintf_s(eventStr, sizeof(eventStr), "%s, #tk#will stop#tk#", eventStr);
				break;
			}
		case prc_act_restart:
			{
				sprintf_s(eventStr, sizeof(eventStr), "%s, #tk#will restart#tk#", eventStr);
				break;
			}
		case prc_act_with_cmd:
			{
				sprintf_s(eventStr, sizeof(eventStr), "%s, #tk#will execute cmd#tk#: %s", eventStr, prcActCmd);
				break;
			}
		case prc_act_unknown:
			{
		...
		pSoftwareMonHandlerContext->monObjInfoList == NULL) return bRet;
	{
		...
		while(curMonObjInfoNode)
		{
							...
							if(bRet)
							{
								if(checkRet)
								{
									...
									if(!bRet)
									{
										...
										{
											sprintf_s(tmpEventMsg, sizeof(tmpEventMsg), "%s,#tk#the act failed#tk#", tmpEventMsg);
										}
									}
									else
									{
										if((prcThrType == prc_thr_type_active && (curMonObjInfoNode->monObjInfo.act > prc_act_unknown && curMonObjInfoNode->monObjInfo.act<=prc_act_with_cmd)))
										{
											sprintf_s(tmpEventMsg, sizeof(tmpEventMsg), "%s,#tk#the act successfully#tk#", tmpEventMsg);
										}
									}
								}
							}
						...
						if(!isPrcMonInfoFind)
						{
							...
							{//delete invalid mon obj(but must keep one<same prcName> exist)
								...
								{*/
									...
									curMonObjInfoNode = curMonObjInfoNode->next;
									if(strlen(repMsg))
										sprintf_s(repMsg, sizeof(repMsg), "%s; Process(PrcName:%s, PrcID:%d) #tk#%s is not exist#tk#", repMsg,tmpMonObjInfoNode->monObjInfo.prcName, 
											tmpMonObjInfoNode->monObjInfo.prcID, tmpMonObjInfoNode->monObjInfo.thrItem.tagName);
									else
										sprintf_s(repMsg, sizeof(repMsg), "Process(PrcName:%s, PrcID:%d) #tk#%s is not exist#tk#", tmpMonObjInfoNode->monObjInfo.prcName, 
											tmpMonObjInfoNode->monObjInfo.prcID, tmpMonObjInfoNode->monObjInfo.thrItem.tagName);
			...
			if(strlen(tmpEventMsg))
			{
				if(strlen(repMsg))
				{
					sprintf_s(repMsg, sizeof(repMsg), "%s;%s", repMsg, tmpEventMsg);
				}
				else
				{
					sprintf_s(repMsg, sizeof(repMsg), "%s",tmpEventMsg);
				}
			}
			app_os_sleep(10);
			curMonObjInfoNode = curMonObjInfoNode->next;
