 ...
 void GetHWM3PartyFunction(void * hHWM3PTYDLL)
{
	if(hHWM3PTYDLL!=NULL)
	{
		pHWM3PARTYInitialize = (PHWM3PartyInitialize)app_get_proc_address(hHWM3PTYDLL, "DllInit");
		pHWM3PARTYUninitialize = (PHWM3PartyUninitialize)app_get_proc_address(hHWM3PTYDLL, "DllUnInit");
		pHWM3PARTYGetValue = (PHWM3PartyGetValue)app_get_proc_address(hHWM3PTYDLL, "HWMGetValue");
		pHWM3PARTYGetPlatformName = (PHWM3PartyGetPlatformName)app_get_proc_address(hHWM3PTYDLL, "DllGetPlatformName");
		pHWM3PartyHWMAvailable = (PHWM3PartyHWMAvailable)app_get_proc_address(hHWM3PTYDLL, "HWMAvailable");
		pHWM3PARTYGetHWMXmlThreshold = (PHWM3PartyGetHWMXmlThreshold)app_get_proc_address(hHWM3PTYDLL, "DllGetHWMXmlThreshold");
		pHWM3PARTYGetHWMXmlPlatform = (PHWM3PartyGetHWMXmlPlatform)app_get_proc_address(hHWM3PTYDLL, "DllGetHWMXmlPlatform");
	}
}
 ...
 bool IsExistHWM3PartyLib()
{
	bool bRet = false;
    void * hHWM3Pty = NULL;
	hHWM3Pty = app_load_library(DEF_HWM3PARTY_LIB_NAME);
	if(hHWM3Pty != NULL)
	{
		bRet = true;
		app_free_library(hHWM3Pty);
		hHWM3Pty = NULL;
	}
	return bRet;
}
...
{
   bool bRet = false;
   hHWM3PARTYDll = app_load_library(DEF_HWM3PARTY_LIB_NAME);
 ...
 bool CleanupHWM3PartyLib()
{
	...
	if(hHWM3PARTYDll != NULL)
	{
		app_free_library(hHWM3PARTYDll);
		hHWM3PARTYDll = NULL;
				...
				while(curNode)
				{
					if(xmlStrcmp(curNode->name, BAD_CAST("Type")) == 0)
					{
						...
						while(curAttr)
						{
							...
							{
								if(_stricmp(type, DEF_SENSORTYPE_TEMPERATURE)==0)
...
bool HWM3PartyGetHWMTempInfo(hwm_info_t * pHWMInfo)
{
	...
	while(curItem)
	{
		if(_stricmp(curItem->type, DEF_SENSORTYPE_TEMPERATURE)==0)
		{
			float value = 0;
...
bool HWM3PartyGetHWMVoltInfo(hwm_info_t * pHWMInfo)
{
	...
	while(curItem)
	{
		if(_stricmp(curItem->type, DEF_SENSORTYPE_VOLTAGE)==0)
		{
			float value = 0;
			int id = atoi(curItem->tag+1);
			if(pHWM3PARTYGetValue(id, &value)<0)
...
bool HWM3PartyGetHWMFanInfo(hwm_info_t * pHWMInfo)
{
	...
	while(curItem)
	{
		if(_stricmp(curItem->type, DEF_SENSORTYPE_FANSPEED)==0)
		{
			float value = 0;
			int id = atoi(curItem->tag+1);
			if(pHWM3PARTYGetValue(id, &value)<0)
...
bool HWM3PartyGetHWMCurrentInfo(hwm_info_t * pHWMInfo)
{
	...
	while(curItem)
	{
		if(_stricmp(curItem->type, DEF_SENSORTYPE_CURRENT)==0)
		{
			float value = 0;
			int id = atoi(curItem->tag+1);
			if(pHWM3PARTYGetValue(id, &value)<0)
...
bool HWM3PartyGetHWMCaseOpenInfo(hwm_info_t * pHWMInfo){
	...
	while(curItem)
	{
		if(_stricmp(curItem->type, DEF_SENSORTYPE_CASEOPEN)==0)
		{
			float value = 0;
			int id = atoi(curItem->tag+1);
			if(pHWM3PARTYGetValue(id, &value)<0)
