...
static net_info_node_t * FindNetInfoNodeWithAdpName(net_info_list netInfoList, char * adpName);
...
static void GetCapability_AutoReport();
...
static CAGENT_PTHREAD_ENTRY(NMAutoUploadThreadStart, args);
...
static BOOL NetMonCheckThr(nm_thr_item_list curThrItemList, net_info_list curNetInfoList, char * checkRetMsg);
...
static void NetMonWhenDelThrCheckNormal(nm_thr_item_list thrItemList, char * checkMsg);
...
static net_info_node_t * FindNetInfoNodeWithIndex(net_info_list netInfoList, int index)
{
	...
	while(findNode)
	{
		if(findNode->netInfo.index == index) break;
		else
		{
...
static int InsertNMThrItemNode(nm_thr_item_list thrList, nm_thr_item_t * pThrItem)
{
...

	{
		...
		{
			strcpy(pThrItem->adapterName, sliceStr[2]);
			strcpy(pThrItem->tagName, sliceStr[3]);
		}

	}

	findNode = FindNMThrItemNodeWithName(head, pThrItem->adapterName, pThrItem->tagName);
	if(findNode == NULL)
	{
		...
		strcpy(newNode->thrItem.adapterName, pThrItem->adapterName);
	...
	if(guidName == NULL || friendlyName == NULL) return bRet;
	{
		...
		if(dwRet == NO_ERROR)
		{
			...
			if(dwRet == NO_ERROR)
			{
				/*char * tmpName = NULL;
				char * utf8Text;
				utf8Text = UnicodeToUTF8((wchar_t *)szFriendName);
				strcpy(friendlyName, utf8Text);
				free(utf8Text);
				free(tmpName);*/
...
BOOL GetNetInfoWithIndex(net_info_t * pNetInfo, DWORD index)
{
	...
	if (app_os_GetIfTable(pIfTable, &dwSize, FALSE) == ERROR_INSUFFICIENT_BUFFER) 
	{
		free(pIfTable);
	...
	if ((dwRetVal = app_os_GetIfTable(pIfTable, &dwSize, FALSE)) == NO_ERROR)
	{
		unsigned int i = 0;
	...
	{
		getWiredStatus(pNetInfo->adapterName, 'a');
	...
	{
		getWirelessStatus(pNetInfo->adapterName, '2');
		if((fp=fopen("out.txt","r+")) == NULL)
		{
			fprintf(stderr, "~~~~ note: %s~~~~ \n", strerror(errno));
			return false;
...
static BOOL GetNetInfo(net_info_list netInfoList)
{
	...
	net_info_list qNetinfo = NULL;
	PIP_ADAPTER_INFO pAdInfo = NULL;
	ULONG ulSizeAdapterInfo = 0;
	DWORD dwStatus;
	int cntAdInfo = 0;
	int cntNetInfoList = 0;

	if(netInfoList == NULL) return bRet;
	ulSizeAdapterInfo = sizeof(IP_ADAPTER_INFO); 
	pAdapterInfo = (PIP_ADAPTER_INFO)malloc(ulSizeAdapterInfo);
	if (app_os_GetAdaptersInfo( pAdapterInfo, &ulSizeAdapterInfo) != ERROR_SUCCESS) 
	{
		free (pAdapterInfo);
		pAdapterInfo = (PIP_ADAPTER_INFO)malloc(ulSizeAdapterInfo);
	}

	dwStatus = app_os_GetAdaptersInfo(pAdapterInfo,   &ulSizeAdapterInfo);

	//if some adapter is disabled.
#if 1
	...
	while(qNetinfo)
	{
		qNetinfo = qNetinfo->next;
		cntNetInfoList++;
	}
	if(cntAdInfo < cntNetInfoList)
	{
		DeleteAllNetInfoNode(netInfoList);
	}
#endif
	...
	memset( &netInfoTemp, 0, sizeof(netInfoTemp));
	...
	fclose(fp);
	//if some adapter is disabled
	if(preAdapterNum > adapterNum)
	{
		DeleteAllNetInfoNode(netInfoList);
	}
	preAdapterNum = adapterNum;
	...
	for(i=0; i<adapterNum; i++)
	{
		...
		ch=fgetc(fp);
		if(ch==EOF || (ch=='\n')) 
			//printf("no wireless netcard.\n");
			;
		else
		{
			//printf("exist wireless netcard.\n");
			while(fgets(str,sizeof(str),fp))
			{
				formatNetStr(str);
...

		{
			//net_info_t * pNetInfo = NULL;
			net_info_node_t * findNode = NULL;
			findNode = FindNetInfoNodeWithIndex(netInfoList, index);
			if(findNode)
			{
				net_info_t netInfo;
...
static CAGENT_PTHREAD_ENTRY(NMAutoReportThreadStart, args)
{
	netmon_handler_context_t *pNetMonHandlerContext = (netmon_handler_context_t *)&NetMonHandlerContext;
	BOOL isAuto = FALSE;
	int intervalMS = 0;
...
static char * GetNMCapabilityString()
{
	...
	{
		char * repJsonStr = NULL;
		int jsonStrlen = Parser_PackCapabilityStrRep(pNetMonHandlerContext->netInfoList, &repJsonStr);
		if(jsonStrlen > 0 && repJsonStr != NULL)
		{
			int len;
...
static void GetCapability()
{
	...
	if(cpbStr != NULL)
	{
		...
		{
			g_sendcbf(&g_PluginInfo, netmon_get_capability_rep, cpbStr, strlen(cpbStr)+1, NULL, NULL);
		}
		else
		{
			//g_sendreportcbf(&g_PluginInfo, cpbStr, strlen(cpbStr)+1, NULL, NULL);
			g_sendcapabilitycbf(&g_PluginInfo, cpbStr, strlen(cpbStr)+1, NULL, NULL); //Capability
		}
		free(cpbStr);
	}
	else
...
static void GetCapability_AutoReport()
{
	...
	if(cpbStr != NULL)
	{
		g_sendcbf(&g_PluginInfo, netmon_auto_report_rep, cpbStr, strlen(cpbStr)+1, NULL, NULL);
		free(cpbStr);
	}
	else
	{
		...
		char errorStr[128];
		int jsonStrlen = 0;
		sprintf(errorStr, "Command(%d), netmon_auto_report_rep error!", netmon_auto_report_rep);
		if(g_sendreportcbfFlag)
			sendLevel = netmon_send_report_cbf;
		SendErrMsg( errorStr, sendLevel);
	}
...
static void SendErrMsg(char * errorStr,int sendLevel)
{
	...
	if(jsonStrlen > 0 && uploadRepJsonStr != NULL)
	{
		...
		case netmon_send_report_cbf:
			...
			//case netmon_send_infospec_cbf:
			//case netmon_cust_cbf:
			//case netmon_subscribe_cust_cbf:
		default:
			{
				g_sendcbf(&g_PluginInfo, netmon_error_rep, uploadRepJsonStr, strlen(uploadRepJsonStr)+1, NULL, NULL);
				break;
...
static BOOL NetMonCheckThrItem(nm_thr_item_t * pThrItemInfo, check_value_type_t valType, float * checkVal, char * checkRetMsg)
{
	...
	else
	{
		if(!pThrItemInfo->isNormal && pThrItemInfo->checkRetValue != DEF_INVALID_VALUE)
		{
			...
			case VT_F:
				{
					sprintf(tmpRetMsg, "%s #tk#normal#tk#", pThrItemInfo->n);
					break;
				}
			}
...
{
	BOOL bRet = FALSE;
	if(curThrItemList == NULL || curNetInfoList == NULL || checkRetMsg == NULL) return bRet;
	{
		...
		while(curThrItemNode)
		{
			if(curThrItemNode->thrItem.isEnable && strlen(curThrItemNode->thrItem.adapterName)>0)
			{
				...
				findNetInfoNode = FindNetInfoNodeWithAdpName(curNetInfoList, curThrItemNode->thrItem.adapterName);
				if(findNetInfoNode == NULL)
				{
					if(strlen(checkRetMsg))sprintf(checkRetMsg, "%s;adapterName:%s not surport", checkRetMsg, curThrItemNode->thrItem.adapterName);
					else sprintf(checkRetMsg, "%s not surport", curThrItemNode->thrItem.adapterName);
	...
	{	
		...
		if(curNetInfoList!=NULL && curNetInfoList->next != NULL && NMThrItemList!=NULL && NMThrItemList->next != NULL)
		{
			...
			if(strlen(NMThrItemList->next->thrItem.adapterName) == 0) 
				NetMonUpdateThr(NULL);
			memset(repMsg, 0, sizeof(repMsg));
			NetMonCheckThr(NMThrItemList, curNetInfoList, repMsg);
		...
		{//Sleep(5000);
			int i = 0;
			for(i = 0; IsNMThrCheckThreadRunning && i<50; i++)
			{
				app_os_sleep(100);
			}
		}
...
static void NetMonWhenDelThrCheckNormal(nm_thr_item_list thrItemList, char * checkMsg)
{
	if(NULL == thrItemList || NULL == checkMsg) return;
	{
		...
		while(curThrItemNode)
		{
			...
			if(curThrItemNode->thrItem.isEnable && !curThrItemNode->thrItem.isNormal)
			{
				...
				sprintf(tmpMsg, "%s<%s> normal", curThrItemNode->thrItem.tagName, curThrItemNode->thrItem.adapterName);
			}
			if(strlen(tmpMsg))
			{
				if(strlen(checkMsg))
...
static void NetMonUpdateThr(nm_thr_item_list newNMThrList)
{
	char repMsg[1024*2] = {0};
	if(newNMThrList != NULL && newNMThrList->next != NULL)
	{
		app_os_mutex_lock(&NMThrInfoMutex);
		//check normal
		NetMonWhenDelThrCheckNormal(NMThrItemList, repMsg);
...
#if TRUE
	{
		...
		if(NMThrItemList != NULL && NMThrItemList->next != NULL && strlen(NMThrItemList->next->thrItem.adapterName) == 0
			&& curNetInfoList != NULL && curNetInfoList->next != NULL)
		{
			nm_thr_item_node_t * curThrItemNode = NMThrItemList->next;
			while(curThrItemNode)
			{
				net_info_node_t * curNetInfoNode = NULL;
				app_os_mutex_lock(&pNetMonHandlerContext->netInfoListMutex);
				curNetInfoNode = curNetInfoList->next;
				strcpy(curThrItemNode->thrItem.adapterName, curNetInfoNode->netInfo.adapterName);
				curNetInfoNode = curNetInfoNode->next;
				while(curNetInfoNode)
				{
					nm_thr_item_t newThrItem;
					memset(&newThrItem, 0, sizeof(nm_thr_item_t));
					strcpy(newThrItem.adapterName, curNetInfoNode->netInfo.adapterName);
					strcpy(newThrItem.tagName, curThrItemNode->thrItem.tagName);
					newThrItem.isEnable = curThrItemNode->thrItem.isEnable;
					newThrItem.maxThr = curThrItemNode->thrItem.maxThr;
					newThrItem.minThr = curThrItemNode->thrItem.minThr;
					newThrItem.thrType = curThrItemNode->thrItem.thrType;
					newThrItem.lastingTimeS = curThrItemNode->thrItem.lastingTimeS;
					newThrItem.intervalTimeS = curThrItemNode->thrItem.intervalTimeS;
					newThrItem.checkType = curThrItemNode->thrItem.checkType;
					InsertNMThrItemNode(NMThrItemList, &newThrItem);
					curNetInfoNode = curNetInfoNode->next;
				}
				app_os_mutex_unlock(&pNetMonHandlerContext->netInfoListMutex);
				curThrItemNode = curThrItemNode->next;
...
static CAGENT_PTHREAD_ENTRY(SetThrThreadStart, args)
{
	...
	{
		NetMonUpdateThr(tmpThrItemList);//in
		app_os_mutex_lock(&NMThrInfoMutex);
...
static void NetMonSetThr(char *thrJsonStr)
{
	if(NULL == thrJsonStr) return;
	{
		...
		{
			FILE * fPtr = fopen(NMThrTmpCfgPath, "wb");
			if(fPtr)
			{
...
static void NetMonDelAllThr()
{
	...
	if(curThrItemList)
	{
		NetMonWhenDelThrCheckNormal(curThrItemList, tmpMsg);
	...
	if(strlen(tmpMsg))
	{
		...
		int jsonStrlen = 0;
		nm_thr_rep_info_t thrRepInfo;
		int repInfoLen = strlen(tmpMsg)+1;
		thrRepInfo.isTotalNormal = TRUE;
		memset(thrRepInfo.repInfo, 0, sizeof(thrRepInfo.repInfo));
...
int HANDLER_API Handler_Start( void )
{
	...
	else
	{
		...
		if (app_os_thread_create(&NMAutoUploadThreadHandle, NMAutoUploadThreadStart, NULL) != 0)//SA3.1 Handler_AutoReportStart
		{
		...
		if (app_os_thread_create(&NMAutoReportThreadHandle, NMAutoReportThreadStart, NULL) != 0)//SA3.1 Handler_Recv
		{
...
void HANDLER_API Handler_Recv( char * const topic, void* const data, const size_t datalen, void *pRev1, void* pRev2  )
{
	...
	case netmon_get_capability_req:
		{
			GetCapability();
			break;
		}
...
void HANDLER_API Handler_AutoReportStart(char *pInQuery)
{
	...
	"NetMonitor":{
	"e":[
	{"n": "NetMonitor/netMonInfoList/本地连接/netUsage"}
	]}
	}}}
	*/
