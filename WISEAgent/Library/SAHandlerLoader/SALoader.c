...
void HANDLERLOADER_API Loader_SetAgentStatus(Handler_List_t *pLoaderList, susiaccess_agent_conf_body_t const * conf, int status)
{
...
void HANDLERLOADER_API Loader_SetFuncCB(Callback_Functions_t* funcs)
{
	...
	if(m_plugininfo)
	{
		if(m_plugininfo->pHandlerInfo)
		{
			HANDLER_INFO_EX *pHandler = m_plugininfo->pHandlerInfo;
...
int HANDLERLOADER_API Loader_LoadAllHandler(Handler_List_t *pLoaderList, char const * workdir)
{
	...
	if(workdir != NULL)
	{
		path_combine(configFile, workdir, MODULE_CONFIG_FILE);
	}
	else
	{
		...
		if( iNum > 0 ) 
		{
			for( i=0; i < iNum; i++ ) 
			{
				...
				if( module_get(configFile, sItemName, sValue, sizeof(sValue)) > 0 ) 
				{
					...
					if(workdir != NULL)
					{
						path_combine(moduleFile, workdir, sValue);
					}
					else
					{
...
static CAGENT_PTHREAD_ENTRY(HandlerReleaseThread, args)
{
...
void HANDLERLOADER_API Loader_ConcurrentReleaseAllHandler(Handler_List_t *pLoaderList)
{
	...
	while(pInterfaceTmp)
	{
		pInterfaceCur = pInterfaceTmp;
		pInterfaceTmp = pInterfaceTmp->prev;
		if (app_os_thread_create(&arrThreadHandler[i], HandlerReleaseThread, pInterfaceCur) != 0)
		{
	...
	for (i = 0; i < total; i++) 
	{
		...
		{
			app_os_thread_join(arrThreadHandler[i]);
...
int Handler_LoadHandler(Handler_Loader_Interface *pLoader, char const *handlerpath, char const *name )
{
	...
	// 1. Load DLL
	handler = app_load_library( handlerpath );

	if( handler == NULL ){
		char* err = app_load_error();
	...
	pLoader->Handler_Init_API				= (HANDLER_INITLIZE) app_get_proc_address( pLoader->Handler, "Handler_Initialize");
	pLoader->Handler_Get_Status_API			= (HANDLER_GET_STATUS) app_get_proc_address( pLoader->Handler, "Handler_Get_Status");	
	pLoader->Handler_OnStatusChange_API		= (HANDLER_ONSTATUSCAHNGE) app_get_proc_address( pLoader->Handler, "Handler_OnStatusChange");
	pLoader->Handler_Start_API				= (HANDLER_START) app_get_proc_address( pLoader->Handler, "Handler_Start");
	pLoader->Handler_Stop_API				= (HANDLER_STOP) app_get_proc_address( pLoader->Handler, "Handler_Stop");
	pLoader->Handler_Recv_API				= (HANDLER_RECV) app_get_proc_address( pLoader->Handler, "Handler_Recv");
	pLoader->Handler_Get_Capability_API		= (HANDLER_GET_CAPABILITY) app_get_proc_address( pLoader->Handler, "Handler_Get_Capability");
	pLoader->Handler_AutoReportStart_API	= (HANDLER_AUTOREPORT_START) app_get_proc_address( pLoader->Handler, "Handler_AutoReportStart");
	pLoader->Handler_AutoReportStop_API		= (HANDLER_AUTOREPORT_STOP) app_get_proc_address( pLoader->Handler, "Handler_AutoReportStop");
	pLoader->Handler_MemoryFree_API			= (HANDLER_MEMORYFREE) app_get_proc_address( pLoader->Handler, "Handler_MemoryFree");
	...
	{
		app_free_library(pLoader->Handler);
...
int Handler_ReleaseHandler(Handler_Loader_Interface *pLoader)
{
	if(!pLoader->Handler)
		return true;
	if(app_free_library(pLoader->Handler) == 0)
	{
