...
static CAGENT_PTHREAD_ENTRY(SetSWMThrThreadStart, args)
{
	...
	else
	{
		...
		if(monObjList) 
		{
			SWMWhenDelThrCheckNormal(monObjList);
...
void SWMWhenDelThrCheckNormal(mon_obj_info_list monObjInfoList) 
{
	if(NULL == monObjInfoList) return;
	{
		...
		while(curNode)
		{
			if(curNode->monObjInfo.thrItem.isEnable && !curNode->monObjInfo.thrItem.isNormal)
			{
				...
				else if(curNode->monObjInfo.prcID < 0)sprintf_s(tmpMsg, sizeof(tmpMsg), "System performance #tk#%s usage back to normal#tk#", //curNode->monObjInfo.prcName, curNode->monObjInfo.prcID, 
...
void SWMDelAllMonObjs()
{
	...
	{
		SWMWhenDelThrCheckNormal(pSWMonHandlerContext->monObjInfoList);
