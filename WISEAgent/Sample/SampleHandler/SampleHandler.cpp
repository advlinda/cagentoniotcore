...
MSG_CLASSIFY_T * CreateCapability()
{
	...
	if(attr)
		IoT_SetStringValue(attr, "%", NULL);

	attr1 = IoT_AddSensorNode(myGroup, "sample");
	if(attr1)
	{
		IoT_SetFloatValueWithMaxMin(attr1, 10, "r", 100, 10, "%");
		g_attr = attr1;
	}
...
static DWORD WINAPI SampleHandlerReportThread(void *args)
{
	...
	while(pHandlerContex->isThreadRunning)
	{
		...
		data = data + ran;
		if(g_attr)
			IoT_SetFloatValueWithMaxMin(g_attr, data, "r", 100, 10, "%");
		if(g_Capability)
...
{
	printf(" %s> Topic:%s, Data:%s\r\n", strHandlerName, topic, (char*)data);
}
...
void HANDLER_API Handler_OnStatusChange( HANDLER_INFO *pluginfo )
{
	...
	sprintf(message, "{\"message\":\"Handler %s on connected\"}", strHandlerName);
	if(g_sendeventcbf)
		g_sendeventcbf(&g_HandlerInfo, Severity_Informational, message, strlen(message), NULL, NULL);
}
